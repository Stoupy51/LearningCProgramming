...
Il existe des instructions pour vérifier la présence ou l'absence de macros, pour définir des macros, pour les remplacer par leur contenu, etc. Voici un aperçu des instructions les plus courantes :
...
<br>Elles permettent également de rendre le code plus portable, c'est-à-dire qu'il pourra être compilé sur différents systèmes d'exploitation sans avoir à modifier le code source si les macros sont bien utilisées.
<br>Lorsqu'on utilisera des fonctionnalités spécifiques à un système d'exploitation, je vous expliquerai comment les rendre portables à l'aide de macros.


## 2. Macros de couleur
Première chose à savoir avant d'entamer cette partie, le compilateur remplace les chaînes de caractères qui se suivent directement par une seule chaîne de caractères, ex : "Bonjour" " " "tout" " " "le" " " "monde" sera remplacé par "Bonjour tout le monde".

Maintenant que vous savez ça, nous allons voir comment colorer le texte affiché dans la console !
Très utile pour afficher des messages d'erreur en rouge, des messages d'avertissement en jaune, etc.
En bref, ça permet de rendre le texte plus lisible !
...
<br>Tous les systèmes UNIX les supportent par défaut, mais Windows ne les supporte qu'à partir de Windows 11.
...
```
Si vous oubliez de mettre `RESET` après une macro de couleur, tout le texte affiché après sera de la même couleur que la macro, même après avoir quitté le programme ! Faites donc attention.


## 3. Fonctions macros
### 3.1. Fonctions macros simples
Il existe un autre type de macros, les fonctions macros.
<br>Elles sont très utiles pour simplifier des tâches qui prennent beaucoup de temps à écrire, comme par exemple afficher un message d'erreur dans la console.

On peut alors définir une fonction macro qui prend en paramètre un message d'erreur et qui affiche ce message d'erreur en rouge dans la console :
```c
...
```
On peut donc imaginer une fonction macro pour chaque type de message (WARNING, INFO, DEBUG, etc.).

Cependant, comment faire si on veut afficher un message d'erreur avec un paramètre, comme par exemple le nom du fichier qui n'a pas pu être ouvert ?
<br>L'utilisation de `printf` semble impossible car il faut pouvoir passer une infinité de paramètres à la fonction macro.
<br>Heureusement, il existe une solution, les macros à nombre variable d'arguments !
...
<br>Vous allez écrire un fichier header `utils.h` qui contiendra des fonctions macros pour afficher des messages d'erreur, d'avertissement, d'information et de débogage avec des couleurs.
<br>Veillez à ce que le programme [`prints.c`](programs/prints.c) compile et s'exécute correctement !
...
<br>Par exemple, vous pouvez définir une macro si vous êtes en mode debug, puis ainsi afficher des messages de débogage uniquement en mode debug.
...
#define DEBUG_STATES (INFO_LEVEL | WARNING_LEVEL | ERROR_LEVEL)
// Ici, DEBUG_STATES vaut 7, car 1 | 2 | 4 = 7 (en binaire : 001 | 010 | 100 = 111)
...
pour ajouter des macros utiles au contrôle d'erreurs de vos programmes.
<br>C'est-à-dire par exemple une macro qui prend en paramètre un code de retour et un message d'erreur, cette macro affiche le message d'erreur si le code de retour est inférieur à 0 et retourne le code de retour.
